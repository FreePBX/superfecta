...
if($usage_mode == 'get caller id')
{
	...
	if($debug)
	{
		print "Searching yellowpages.com.au with google... ";
	}
...
//		$url = "http://www.google.com/search?num=30&hl=en&lr=&safe=off&as_qdr=all&q=%22".$num1."+".$num2."+".$num3."%22+site%3Awww.yellowpages.com.au+-site%3Awww.yellowpages.com.au%2Fsearch&btnG=Search";   //working Dec 2010
		$url = "http://www.google.com/search?q=(".$num1.")+".$num2."+".$num3."+-similar+site%3Ayellowpages.com.au&amp;ie=utf-8&amp;oe=utf-8&amp;aq=t&amp;rls=org.mozilla:en-US:official&amp;client=firefox-a";  //working May 31, 2012

		$value = get_url_contents($url);

		// Check if we can just pull the name directly from google without having to pull the slow yellopages.com.au page
//		$pattern = "/<a href=\"http:\/\/www\.yellowpages\.com\.au\/(?!find|search)[^\"]{1,100}\"[^>]{1,100}>([^-<]{1,100}).{1,100}<div class=\"s\">[^:<]{1,200}<em>".$num1.".{0,1} ".$num2." ".$num3."/i";   //working as of Dec 2010
		$pattern ="/<a href=\"\/url\?q=http:\/\/www\.yellowpages\.com\.au\/[^f].+?\">(.{3,40}?) - .{1,200}$num1.{0,20}$num2.{0,10}$num3<\/b>\./";    //working as of May 31, 2012
...
/*****  Temporarily removing the deep search section since it is not working as of May 31, 2012 *******
